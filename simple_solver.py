...
class Cell:
    """
    A single sudoku cell with coordinates x,y.
    """
...
class Board:
    """
    A sudoku board of 9x9 Cell objects.
    """
    ...
    def solve_one(self, get_group: Callable) -> None:
        ...
        assert self.check_board(), "Error in board!"
        for i in range(9):
...
def init_board(board_obj: Board, data:list) -> None:
    ...
    assert all(len(item) == 9 for item in data), "Input data must have 9 items in each string."
    for i in range(9):
        rowdata = list(data[i])
