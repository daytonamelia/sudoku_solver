...
def test_solve_completes(puzzle: dict[str, list[str]]) -> None:
    """solve leaves no empty cells on a solvable puzzle."""
    board = Board(len(puzzle['unsolved']))
...
@pytest.mark.parametrize("puzzle", PUZZLES.values(), ids=PUZZLES.keys())
def test_solve_correct(puzzle: dict[str, list[str]]) -> None:
...
def test_solve_board_valid(solved_board: Board) -> None:
